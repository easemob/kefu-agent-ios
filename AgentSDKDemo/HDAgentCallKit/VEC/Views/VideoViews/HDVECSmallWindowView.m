...
#import "HDVECCallCollectionViewCell.h"
#define fDeviceWidth ([UIScreen mainScreen].bounds.size.width)
#define fDeviceHeight ([UIScreen mainScreen].bounds.size.height)
#define kPoirtSpace  10
@interface HDVECSmallWindowView()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>{
    NSMutableArray *_cellArray;     //collectionView数据
}
@property (nonatomic, strong) UICollectionViewFlowLayout *flowLayout;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) HDVECCallCollectionViewCell *currentCell;
@end
@implementation HDVECSmallWindowView
...
- (void)setItemData:(NSArray<HDVECCallCollectionViewCellItem *> *)items{
...
- (void)setSelectCallItemChangeVideoView:(HDVECCallCollectionViewCellItem *)item withIndex:(NSInteger)index{
...
- (BOOL)setThirdUserdidJoined:(HDVECCallCollectionViewCellItem *)item{
    BOOL isNeedAdd = YES;
    @synchronized(_cellArray){
        for (HDVECCallCollectionViewCellItem * tt in _cellArray) {
            if (tt.uid  == item.uid) {
                isNeedAdd = NO;
                break;
            }
...
- (void)removeCurrentCellItem:(HDVECCallCollectionViewCellItem *)item{
...
- (void)addCurrentCellItem:(HDVECCallCollectionViewCellItem *)item{
...
//        flowLayout.sectionInset = UIEdgeInsetsMake(0, 25, 5, 25);//上左下右
        ...
        [_collectionView registerClass:[HDVECCallCollectionViewCell class] forCellWithReuseIdentifier:@"HDCallCollectionViewCell"];
        //设置代理
...

{
    ...
    HDVECCallCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identify forIndexPath:indexPath];
    [cell sizeToFit];
...
- (void)setAudioMuted:(HDVECCallCollectionViewCellItem *)item{
...

{
    ...
    self.currentCell = (HDVECCallCollectionViewCell *)  [self.collectionView cellForItemAtIndexPath:indexPath];
