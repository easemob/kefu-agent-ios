...
typedef NS_ENUM(NSInteger,HDVECButtonDirection){
    HDVECButtonDirectionLeft    =0,
    HDVECButtonDirectionRight   =1,
    HDVECButtonDirectionTop     =3,
    HDVECButtonDirectionBottom  =4
};
/**
 ...
 */
@implementation HDVECSuspendButton
...
@end
@implementation HDVECSuspendScrollView
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    [[self nextResponder]touchesBegan:touches withEvent:event];
}
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
...
@end
@implementation HDVECSuspendImageView
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    [[self nextResponder]touchesBegan:touches withEvent:event];
}
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
...
@end
@implementation HDVECSuspendView
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    [[self nextResponder]touchesBegan:touches withEvent:event];
}
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
...
@interface HDVECSuspendCustomView()<UIScrollViewDelegate>
{
    AVPlayer *_player;
...
@implementation HDVECSuspendCustomView
...
- (void)createCustomMusic{
    if (!_customContentView) {
        _customContentView=[[HDVECSuspendView alloc]init];
...
- (void)createCustomScrollView{
    if (!_customScrollView) {
        _customScrollView=[[HDVECSuspendScrollView alloc]init];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    HDVECButtonDirection direction = HDVECButtonDirectionLeft;
    CGFloat minDistance = left;
    if (right < minDistance) {
        minDistance = right;
        direction = HDVECButtonDirectionRight;
    }
    if (top < minDistance) {
        minDistance = top;
        direction = HDVECButtonDirectionTop;
    }
    if (bottom < minDistance) {
        direction = HDVECButtonDirectionBottom;
    }
    ...
    switch (direction) {
        ...
        case HDVECButtonDirectionRight:
        {
            [UIView animateWithDuration:0.3 animations:^{
        ...
        case HDVECButtonDirectionBottom:
        {
            [UIView animateWithDuration:0.3 animations:^{
