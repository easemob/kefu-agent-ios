...
//    [self.cellScrollView addGestureRecognizer:self.longPressGestureRecognizer];
    ...
    [_openBtn setTitleColor:RGBACOLOR(0x4a, 0x90, 0xe2, 1) forState:UIControlStateNormal];
    [_openBtn addTarget:self action:@selector(openQuickReply:) forControlEvents:UIControlEventTouchUpInside];
    _openBtn.hidden = YES;
    [self.contentView addSubview:_openBtn];
    
    UILongPressGestureRecognizer * longPressGr = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longPressToDo:)];
    longPressGr.minimumPressDuration = 0.5;
    [self.cellScrollView addGestureRecognizer:longPressGr];
}
...
- (void)setQuickReplyMessageModel:(QuickReplyMessageModel*)model
{
    ...
    CGRect rect = [model.phrase boundingRectWithSize:CGSizeMake(KScreenWidth - 40.f, MAXFLOAT)
                                              options:NSStringDrawingUsesLineFragmentOrigin
                                           attributes:attributes
                                              context:nil];
    CGFloat height = CGRectGetHeight(rect) + 35.f;
    self.openBtn.hidden = height > 94.f?NO:YES;
}
...
- (void)layoutSubviews
{
    ...
    if (self.openBtn.hidden) {
        self.textLabel.frame = CGRectMake(20, 16.f, frame.size.width - 40, frame.size.height - 32.f);
    } else {
        self.textLabel.frame = CGRectMake(20, 16.f, frame.size.width - 40, frame.size.height - 64.f);
    }
    self.textLabel.numberOfLines = CGRectGetHeight(self.textLabel.frame)/15.f;
    self.textLabel.font = [UIFont systemFontOfSize:15.0f];
    self.textLabel.textColor = RGBACOLOR(0x4a, 0x4a, 0x4a, 1);
    
    self.openBtn.frame = CGRectMake(20, frame.size.height - 32.f, 40.f, 16.0f);
}
...
- (void)openQuickReply:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(openQuickReplyToReloadView)]) {
        ...
        [self.delegate openQuickReplyToReloadView];
    }
}
...
-(void)longPressToDo:(UILongPressGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        if ([self.delegate respondsToSelector:@selector(longPressSwipeableTableViewCell:)]) {
            [self.delegate longPressSwipeableTableViewCell:self.model];
        }
    }
}
...
+ (CGFloat)heightForCell:(QuickReplyMessageModel *)model
{
    if (model.isOpen) {
        ...
        CGRect rect = [model.phrase boundingRectWithSize:CGSizeMake(KScreenWidth - 40.f, MAXFLOAT)
                                              ...
                                              attributes:attributes
                                                 context:nil];
        CGFloat height = CGRectGetHeight(rect) + 35.f + 32.f;
        return height<54.f?54.f:height;
    } else {
        NSDictionary *attributes = @{NSFontAttributeName :[UIFont systemFontOfSize:15.0f]};
        CGRect rect = [model.phrase boundingRectWithSize:CGSizeMake(KScreenWidth - 40.f, MAXFLOAT)
                                                  options:NSStringDrawingUsesLineFragmentOrigin
        ...
        if (height < 54.f) {
            return 54.f;
        } else if (height >= 54.f && height <96.f) {
            return height;
        } else {
            return 94.f + 32.f;
        }
