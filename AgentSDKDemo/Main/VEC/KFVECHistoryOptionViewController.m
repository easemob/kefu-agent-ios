//
//  KFVECHistoryOptionViewController.m
//  AgentSDKDemo
//
...
@interface KFVECHistoryOptionViewController () <EMPickerSaveDelegate,EMChatHeaderTagViewDelegate,EMEditViewControllerDelegate,EMTimePickerViewDelegate>
{
    NSArray *_timeArray;
...
@implementation KFVECHistoryOptionViewController
...
- (void)initData
{
    ...
    if (_type == VECEMHistoryOptionType) {
//        _optionArray = @[@"渠道",@"关联",@"标签",@""];
    ...
    _originTypeArray = @[@{@"key":@"全部渠道",@"value":@""},@{@"key":@"网页",@"value":@"webim"},@{@"key":@"APP",@"value":@"app"},@{@"key":@"微信",@"value":@"weixin"},@{@"key":@"微博",@"value":@"weibo"}
                         ...
                         ,@{@"key":@"BCP",@"value":@"bcp"}
                         ,@{@"key":@"钉钉",@"value":@"dingtalk"}
                         ,@{@"key":@"抖音",@"value":@"douyin"}
                         ,@{@"key":@"头条",@"value":@"toutiao"}
                         ,@{@"key":@"快手",@"value":@"kuaishou"}
                         ,@{@"key":@"FACEBOOK",@"value":@"facebook"}
                         ,@{@"key":@"INSTRAGRAM",@"value":@"instragram"}
...
//    _tagArray = @[@{@"key":@"全部会话",@"value":@""},@{@"key":@"全部会话标签",@"value":[self _getAllRootNode]},@{@"key":@"未指定会话标签",@"value":@"0"}];
    ...
    if (_startDate) {
        [_parameters hd_setValue:[self formatDate:_startDate] forKey:@"createDateFrom"];
    }
    if (_endDate) {
        [_parameters hd_setValue:[self formatDate:_endDate] forKey:@"createDateTo"];
    }
    
    NSArray * state = @[@"Processing",@"Terminal",@"Abort"];
    [_parameters hd_setValue:state forKey:@"state"];
    UserModel  *user = [HDClient sharedClient].currentAgentUser;
    NSString * orginTypeStr =[[_originTypeArray objectAtIndex:_originType] valueForKey:@"value"];
    if (orginTypeStr.length > 0) {
        NSArray * originType = @[orginTypeStr];
        [_parameters hd_setValue:originType forKey:@"originType"];
    }
 
    [_parameters hd_setValue:user.tenantId forKey:@"tenantId"];
    [_parameters hd_setValue:user.agentId forKey:@"agentUserId"];
    if (_visitorName.length > 0) {
        [_parameters hd_setValue:_visitorName forKey:@"visitorName"];
    }
...
- (void)saveStartDate:(NSDate *)startDate endDate:(NSDate *)endDate
{
    ...
    [self updateParameters:@"createDateFrom" value:[self formatDate:_startDate]];
    _endDate = endDate;
    [self updateParameters:@"createDateTo" value:[self formatDate:_endDate]];
    _timeIndex = 5;
...
- (void)savePickerWithValue:(NSString *)value index:(NSInteger)index
{
    if (_curSection == 0) {
        if (_curIndexRow == 0) {
            ...
            if (_startDate) {
                [self updateParameters:@"createDateFrom" value:[self formatDate:_startDate]];
                [_parameters setObject:[self formatDate:_startDate] forKey:@"createDateFrom"];
            }
            if (_endDate) {
                [self updateParameters:@"createDateTo" value:[self formatDate:_endDate]];
            }
        ...
        if (_curIndexRow == 0) {
            ...
            if (orginTypeStr.length > 0) {
                NSArray * originType = @[orginTypeStr];
                [_parameters hd_setValue:originType forKey:@"originType"];
            [self updateParameters:@"originType" value:originType];
            }
...
- (void)optionAction
{
    if (self.optionDelegate && [self.optionDelegate respondsToSelector:@selector(vecHistoryOptionWithParameters:)]) {
        [self.optionDelegate vecHistoryOptionWithParameters:_parameters];
    }
}
...
//            [MBProgressHUD show:@"已存在" view:self.view];
            [MBProgressHUD showMessag:@"已存在" toView:self.view];
        }
    }
    [self.navigationController popToViewController:self animated:YES];
}
...
- (void)updateParameters:(NSString *)key value:(id)value
{
    if (_parameters) {
        
        [_parameters hd_setValue:value forKey:key];
    }
}
