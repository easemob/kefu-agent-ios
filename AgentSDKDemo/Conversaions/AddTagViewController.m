...
- (EMChatHeaderTagView*)headerTagView
{
    if (_headerTagView == nil) {
        _headerTagView = [[EMChatHeaderTagView alloc] initWithSessionId:_serviceSessionId edit:YES];
        ...
        if (!error) {
            ...
            for (NSString *string in json) {
                ...
                if ([weakSelf.tree objectForKey:key]) {
                    [array addObject:[weakSelf.tree objectForKey:key]];
                }
            }
    ...
    [[HDClient sharedClient].chatManager asyncGetSessionCommentWithSessionId:_serviceSessionId completion:^(id responseObject, HDError *error) {
        if (!error) {
            ...
            if (json != nil) {
                ...
                [weakSelf.commentDic setObject:weakSelf.serviceSessionId forKey:@"serviceSessionId"];
            }
        }
    }];
}
